...
import { assertValidParentFolder } from '$lib/server/validate.js';
...
export const POST: RequestHandler = async ({ params, request, locals }) => {
	...
	// A bogus or trashed parent produces a node that's invisible in the
	// tree (same failure class as P1-3) — reject before it's created.
	assertValidParentFolder(locals.db, params.id, body.parent_id);
	...
	if (body.type === 'folder') {
		...
		const folder = locals.db.prepare('SELECT * FROM folders WHERE id = ?').get(id) as Record<string, unknown>;
		...
		const doc = locals.db.prepare('SELECT * FROM documents WHERE id = ?').get(id) as Record<string, unknown>;
